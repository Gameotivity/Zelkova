...
function useCountUp(end: number, duration = 2000, start = 0, active = true) {
  ...
  }, [end, duration, start, active, hasAnimated]);
  return value;
}
...
function ToastPopups({ trades }: { trades: ReturnType<typeof generateTrades> }) {
  ...
  return (
    <div className="fixed bottom-4 right-4 z-50 flex max-w-xs flex-col gap-2 pointer-events-none">
      {toasts.map((t) => (
  ...
  return (
    <nav className={cn("fixed top-0 z-50 w-full transition-all duration-300", scrolled ? "border-b border-white/10 bg-[#06080E]/95 backdrop-blur-2xl shadow-lg" : "bg-transparent")}>
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-6">
        ...
        <div className="flex items-center gap-3">
          <button onClick={() => setMobileOpen(!mobileOpen)} className="flex h-10 w-10 items-center justify-center rounded-lg text-[#94A3B8] transition-colors hover:bg-white/10 hover:text-white md:hidden" aria-label="Toggle menu">
            <svg className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>{mobileOpen ? <path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" /> : <path strokeLinecap="round" strokeLinejoin="round" d="M3.75 6.75h16.5M3.75 12h16.5m-16.5 5.25h16.5" />}</svg>
          </button>
          <Link href="/dashboard" className="hidden items-center gap-2 rounded-lg bg-[#00E5FF] px-5 py-2 text-sm font-bold text-[#06080E] transition-all hover:shadow-lg hover:shadow-[#00E5FF]/25 hover:-translate-y-0.5 sm:flex">
      ...
      {mobileOpen && (
        <div className="border-t border-white/10 bg-[#06080E]/95 backdrop-blur-2xl md:hidden">
          <div className="flex flex-col gap-1 px-6 py-4">
            <Link href="#pipeline" onClick={() => setMobileOpen(false)} className="rounded-lg px-4 py-3 text-sm text-[#94A3B8] transition-colors hover:bg-white/5 hover:text-white">How It Works</Link>
            <Link href="#performance" onClick={() => setMobileOpen(false)} className="rounded-lg px-4 py-3 text-sm text-[#94A3B8] transition-colors hover:bg-white/5 hover:text-white">Performance</Link>
            <Link href="#security" onClick={() => setMobileOpen(false)} className="rounded-lg px-4 py-3 text-sm text-[#94A3B8] transition-colors hover:bg-white/5 hover:text-white">Security</Link>
            <Link href="/pricing" onClick={() => setMobileOpen(false)} className="rounded-lg px-4 py-3 text-sm font-semibold text-[#00E5FF] transition-colors hover:bg-white/5">Pricing</Link>
            <Link href="/dashboard" onClick={() => setMobileOpen(false)} className="mt-2 rounded-lg bg-[#00E5FF] px-4 py-3 text-center text-sm font-bold text-[#06080E]">Connect Wallet</Link>
          </div>
        </div>
      )}
    </nav>
      ...
      <section className="relative flex min-h-screen items-center justify-center overflow-hidden pt-28">
        ...
        <div className="relative z-10 mx-auto max-w-6xl px-6 text-center">
          ...
          <h1 className="animate-fade-in-up stagger-1 text-5xl font-black leading-[1.05] tracking-tight sm:text-7xl md:text-[5.5rem]">
            ...
            <br /><span className="whitespace-nowrap text-[#10B981]">Real Profits.</span>
          </h1>

          <p className="mx-auto mt-8 max-w-xl animate-fade-in-up stagger-2 text-lg leading-relaxed text-[#94A3B8] md:text-xl">
